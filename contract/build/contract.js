...
let PaintoContract = (_dec = NearBindgen({}), _dec2 = initialize({}), _dec3 = call({
  payableFunction: true
}), _dec4 = view({}), _dec5 = view({}), _dec6 = view({}), _dec(_class = (_class2 = class PaintoContract {
  sponsorships = new UnorderedMap("map-uid-1");
  ...
  sponsor({
    ...
    let sponsor = predecessorAccountId();
    let donationAmount = attachedDeposit();
    let sponsoredSoFar = this.sponsorships.get(sponsor) === null ? BigInt(0) : BigInt(this.sponsorships.get(sponsor));
  ...
  get_sponsorship_for_account({
    ...
    return new Sponsor({
      account_id,
      beneficiary_id,
      total_amount: this.sponsorships.get(account_id)
    });
  }
...
function get_sponsorship_for_account() {
  ...
  let _contract = PaintoContract._create();

  if (_state) {
    PaintoContract._reconstruct(_contract, _state);
  }
...
function get_sponsorships() {
  ...
  let _result = _contract.get_sponsorships(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(PaintoContract._serialize(_result));
}
function number_of_sponsorships() {
  let _state = PaintoContract._getState();
  ...
  let _result = _contract.number_of_sponsorships(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(PaintoContract._serialize(_result));
}
function sponsor() {
  let _state = PaintoContract._getState();
  ...
  let _result = _contract.sponsor(_args);

  PaintoContract._saveToStorage(_contract);
